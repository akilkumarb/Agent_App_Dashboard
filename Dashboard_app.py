...
# If email is not submitted, show input field
if not st.session_state.user_email:
    email = st.text_input("Enter your Razorpay email-id to continue:")
    if st.button("Submit"):
        st.session_state.user_email = email  # Store email in session state
        st.rerun()  # Rerun the app to hide the input box

# If email is submitted, proceed
if st.session_state.user_email:
    user_email = st.session_state.user_email  # Use stored email
    st.write(f"Welcome, **{user_email}**!")

    if user_email in ALLOWED_UPLOADERS:
    ...
    if device_order_data is not None and fos_data is not None:
        ...
        | ((final_data_with_subtotals["BH_Name"].isin(bh)) & (final_data_with_subtotals["Reporting Manager"] == "Subtotal"))  
